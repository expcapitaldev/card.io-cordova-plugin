...
- (void)scan:(CDVInvokedUrlCommand *)command {
    ...
    paymentViewController.maskManualEntryDigits = YES;
    ...
    [pvc dismissViewControllerAnimated:YES completion:^{
      ...
      if(info.cardImage != nil) {
          dispatch_async(dispatch_get_global_queue(QOS_CLASS_BACKGROUND, 0), ^{
              CGRect rect = CGRectMake(0, info.cardImage.size.height / 2, info.cardImage.size.width / 1.4, info.cardImage.size.height / 6);
              NSString *base64Img = [UIImagePNGRepresentation([self drawRectangleOnImage:info.cardImage rect: rect]) base64EncodedStringWithOptions:0];
              dispatch_async(dispatch_get_main_queue(), ^{
                    [response setObject:base64Img forKey:@"img"];

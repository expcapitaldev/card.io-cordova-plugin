...
public class CardIOCordovaPlugin extends CordovaPlugin {
        ...
        if (REQUEST_CARD_SCAN == requestCode) {
            if (resultCode == CardIOActivity.RESULT_CARD_INFO) {
                ...
                if (intent.hasExtra(CardIOActivity.EXTRA_SCAN_RESULT)) {
                    ...
                    try {
                        Bitmap image = CardIOActivity.getCapturedCardImage(intent);
                    ...
                    } catch (OutOfMemoryError o) { // In case card.io crashed
                    } finally {
                        // At least return text data
                        this.callbackContext.success(this.toJSONObject(scanResult, encodedImage));
                    }
                } else {
    ...
    private JSONObject toJSONObject(CreditCard card, String base64Image) {
